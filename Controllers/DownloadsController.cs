...
namespace MirrorServer.Controllers
{
    ...
    public class DownloadsController : Controller
    {
        ...
        {
            Resource result = _context.Resources.SingleOrDefault(resource => resource.Name.Equals(name, StringComparison.InvariantCultureIgnoreCase));
            if (result == null)
            {
                return NotFound();
            }
            ...
            ResourceVersion version = result.Versions.Where(version => version.Qualifier.Equals("RELEASE", StringComparison.OrdinalIgnoreCase)).OrderByDescending(version => version.BuildNumber).FirstOrDefault();
            if (version == null)
            {
                return NotFound();
            }
